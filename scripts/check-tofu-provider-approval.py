...
def main():
    ...
    if blocked:
        ...
        print()
        print("  Full procedure: docs/runbooks/tofu-provider-bumps.md")
        print()
        print("  To proceed:")
        ...
        print("    3. read the plan itself, not just the summary:")
        print("         kubectl get secret -n tofu tfplan-default-<module>-config \\")
        print("           -o jsonpath='{.data.tfplan}' | base64 -d | gunzip -c > tfplan")
        print("         tofu show tfplan     # must be the runner's tofu version, 1.12.1")
        print("    4. approve OUT OF BAND -- not through git. The plan id embeds the source")
        print("       sha, so an approval commit invalidates the plan it names:")
        print("         P=$(kubectl get terraform -n tofu <module>-config \\")
        print("              -o jsonpath='{.status.plan.pending}')")
        print("         kubectl patch terraform -n tofu <module>-config --type=merge \\")
        print("           -p \"{\\\"spec\\\":{\\\"approvePlan\\\":\\\"$P\\\"}}\"")
        print("    5. set approvePlan back to auto -- manual is a state you leave")
        print()
